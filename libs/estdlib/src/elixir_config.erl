...
static(Map) when is_map(Map) ->
    ...
    CurrentEnv = persistant_term(),
    MergedMap = maps:merge(CurrentEnv, Map),

    ets:insert('persistant_term', {env, MergedMap}).

is_bootstrap() ->
  maps:get(bootstrap, persistant_term(), false).
identifier_tokenizer() ->
  maps:get(identifier_tokenizer, persistant_term(), 'Elixir.String.Tokenizer').

%% Key-value store (concurrent reads, serial writes)

persistant_term() ->
  case ets:lookup('persistant_term', env) of
        [{env, CurrentMap}] when is_map(CurrentMap) -> 
            CurrentMap;
        _ -> 
            #{}
    end.
